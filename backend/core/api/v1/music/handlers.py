...
class TrackView(viewsets.ModelViewSet):
    ...
    def perform_destroy(self, instance):
        ...
        file = cover_field.default() if callable(cover_field.default) else cover_field.default
    ...
    def partial_update(self, request, *args, **kwargs):
        ...
        if user_of_likes_data is not None:
            ...
            try:
                ...
                user_ids = [int(id) for id in user_of_likes_data] if user_of_likes_data else []
...
class UsersCollectionView(viewsets.ModelViewSet):
    ...
    def partial_update(self, request, *args, **kwargs):
        ...
        # Проверяем входящие данные
        track_ids = request.data.getlist('track_ids') if 'track_ids' in request.data else None
        # Получаем track_ids
        print(f"request.data: {request.data}")  # Для отладки
...
def like_tracks_list(request):
    if request.method == 'GET':
        liked_collections = AuthorsCollection.objects.filter(user_of_likes__id=request.user.id)
